...
void Oscilloscope_Show_Wave(void)
{
	...
	if(HAL_GetTick() >= Show_Wave_Tick && ADC_Samp_Ok == 1)
	{
		...
		if(Scan_Show == 0)
		{
			if(Tri_Pos[0] > ADC_Length - 231) Tri_Pos[0] = ADC_Length - 231;
			if(Tri_Pos[1] > ADC_Length - 231) Tri_Pos[1] = ADC_Length - 231;
			
			//生成绘图数据
			for(uint16_t i = 0;i < 230;i++)
			{
				float Voltage_CH1 = (((ADC_Show[i + Tri_Pos[0]][0] * 3.3) / 4096.0) - 1.645875) / 0.33;
				float Voltage_CH2 = (((ADC_Show[i + Tri_Pos[1]][1] * 3.3) / 4096.0) - 1.645875) / 0.33;
				
				Show_Buffer[i][Show_Cnt][0] = (int32_t)(Voltage_CH1 * (23.0 / Vlotage_Bisic[Voltage_Div])) + 114;//通道1绘图数据
				//                                                     ^ (205 - 23) / 8 格 = 22.75像素，取23像素，这是波形背景格子的垂直方向像素数
		...
		else
		{
			...
			if(Scan_Cnt_Buffer > 2)
			{
				for(uint16_t i = 0;i < Scan_Cnt_Buffer - 2;i++)
				{
					...
					LCD_DrawLine(i + 6,Show_Buffer[i][0][0],
							i + 7,Show_Buffer[i + 1][0][0],YELLOW);
					LCD_DrawLine(i + 6,Show_Buffer[i][0][1],
							i + 7,Show_Buffer[i + 1][0][1],CYAN);
				}
	...
	if(HAL_GetTick() >= Oscilloscope_Show_Tick)//200ms更新一次示波器信息
	{
		...
		for(uint8_t i = 0;i < 2;i++)
		{
			...
			if(Wave_Freq[i])
			{
				...
				sprintf(Str_Buffer,"%-9s",Str); // 写错了
			}
			else
				sprintf(Str_Buffer,"F=  --  "); // // 写错了
			
			if(i == 0)
				Show_Str(171,207,YELLOW,BLACK,Str_Buffer,0);
			else
				Show_Str(171,224,CYAN,BLACK,Str_Buffer,0); // 难怪第二路不能显示频率
		}
	}
}
	...
	for(uint8_t Ch = 0;Ch < 2;Ch++)
	{
		for(uint16_t i = 0;i < 230;i++)
		{
			...
			if(i < ADC_Length - 2 && i > 1)//查找触发位置
			{
				...
				{
					if(ADC_Scan_Samp[i - 2][Ch] >= Tri_Voltage && ADC_Scan_Samp[i + 2][Ch] <= Tri_Voltage)
					{
						if(Tri_Flag[Ch] == 0)
						{
							Tri_Pos_Buffer[Ch] = i;
...
float DC1_Voltage = 3.3;
float DC2_Voltage = 3.3;
uint8_t DC1_State = 1;
uint8_t DC2_State = 1;
