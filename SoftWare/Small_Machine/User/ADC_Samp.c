...
uint16_t Best_Tri[2] = {0};			//最佳触发电压 VPP/2
uint16_t Tri_Pos[2] = {0};			//触发位置
	...
	for(uint8_t Ch = 0;Ch < 2;Ch++)
	{
		for(uint16_t i = 0;i < ADC_Length;i++)
		{
			...
			if(i < ADC_Length - 2 && i > 1)//查找触发位置
			{
				...
				{
					if(ADC_Show[i - 2][Ch] >= Tri_Voltage && ADC_Show[i + 2][Ch] <= Tri_Voltage)
					{
						if(Tri_Flag[Ch] == 0)
						{
							Tri_Pos_Buffer[Ch] = i;
...
void ADC_Data_Handle(void)
{
	...
	for(uint8_t Ch = 0;Ch < 2;Ch++)
	{
		for(uint16_t i = 0;i < ADC_Length;i++)
		{
			if(Tri_Flag == 0 && i < ADC_Length - 2 && i > 1)//查找触发位置
			{
				if(Tri_Edge == Edge_Rise)//上升沿触发
				{
					...
					if(ADC_Show[i - 1][Ch] > Tri_Voltage && ADC_Show[i + 1][Ch] < Tri_Voltage)
					{
						if(ADC_Show[i - 2][Ch] > Tri_Voltage && ADC_Show[i + 2][Ch] < Tri_Voltage)
						{
							Tri_Pos[Ch] = i;
		...
		Best_Tri[Ch] = (ADC_Max[Ch] + ADC_Min[Ch]) / 2.0;
	}
	Get_Wave_Cycle();
